...
3. **Copy all `.py` files from `proj1` to the top-level directory (keeping file names unchanged):**
   cp /home/activity2/proj1/*.py /home/activity2/
4. **Copy `p1.txt` from `proj1` to `proj2` as `p1_new.txt` (from top-level directory):**
   cp /home/activity2/proj1/p1.txt /home/activity2/proj2/p1_new.txt
5. **Create a subdirectory `proj3` under the top-level directory and copy all contents of `proj1` to it:**
   mkdir /home/activity2/proj3
cp -r /home/activity2/proj1/* /home/activity2/proj3/
6. **Run `p1.py` Flask application in the background and keep it running after disconnecting from the terminal:**
